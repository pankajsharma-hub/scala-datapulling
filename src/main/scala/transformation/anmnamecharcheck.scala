...
m.toString
  ...
  def main(args: Array[String]): Unit = {
     ...
     val conf = new SparkConf().setAppName("NameChar Check")
     Class.forName("com.mysql.jdbc.Driver")
    val sc = new SparkContext(conf)
    val sqlContext=new org.apache.spark.sql.SQLContext(sc)
    import sqlContext.implicits._
    val anm = sqlContext.read.format("delta").load("hdfs://masterHP:9000/delta/warehouse/HP_test/staging/anm_details_stage2")
    anm.registerTempTable("anm_master2")
    
    //Transformed_Records

    val count=anm.filter(col("anm_name").contains("@")||col("anm_name").contains("!")||col("anm_name").contains("#")||col("anm_name").contains("$")||col("anm_name").contains("%")||col("anm_name").contains("^")||col("anm_name").contains("&")||col("anm_name").contains("*")).count
...
//    Transformed_data.write.mode(SaveMode.Append).jdbc("jdbc:mysql://masterTN/nrp", "PreProcessed_Table", mysql_props1)
    ...
    val ECM= sqlContext.sql("Select * from anm_master2")
    val namecheck = ECM.withColumn("anm_name", when($"anm_name".isNotNull, Name_char($"anm_name")))

if(!namecheck.isEmpty){
          namecheck.write.format("delta").save("hdfs://masterHP:9000/delta/warehouse/HP_test/validation/nrp_anm_master")
}
  }
}
