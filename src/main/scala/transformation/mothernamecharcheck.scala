...
m.toString
  ...
  def main(args: Array[String]): Unit = {
     ...
     val conf = new SparkConf().setAppName("Mothername Check")
     Class.forName("com.mysql.jdbc.Driver")
    val sc = new SparkContext(conf)
    val sqlContext=new org.apache.spark.sql.SQLContext(sc)
    import sqlContext.implicits._
    val mother = sqlContext.read.format("delta").load("hdfs://masterHP:9000/delta/warehouse/HP_test/staging/mother_master_stage2")
    mother.registerTempTable("mother_master2")

//Transformed_Records

val count=mother.filter(col("mother_name").contains("@")||col("mother_name").contains("!")||col("mother_name").contains("#")||col("mother_name").contains("$")||col("mother_name").contains("%")||col("mother_name").contains("^")||col("mother_name").contains("&")||col("mother_name").contains("*")).count
...
//    Transformed_data.write.mode(SaveMode.Append).jdbc("jdbc:mysql://masterTN/nrp", "PreProcessed_Table", mysql_props1)
    ...
    val ECM= sqlContext.sql("Select * from mother_master2")
    val namecheck = ECM.withColumn("mother_name", when($"mother_name".isNotNull, Name_char($"mother_name")))
    if(!namecheck.isEmpty){
          namecheck.write.format("delta").save("hdfs://masterHP:9000/delta/warehouse/HP_test/validation/nrp_mother_master")
    }
  }
}
